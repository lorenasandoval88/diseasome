...
function renderParticipantsTable(list, targetId, title, key) {
	...
	const renderPage = () => {
		...
		const rowsHtml = pageItems.map((p, i) => {
			...
			// Multiple files per participant: reflect the active file and allow paging through them.
			const pFiles = Array.isArray(p.files) ? p.files : [];
			const multiFile = pFiles.length > 1;
			if (multiFile) {
				if (p._activeFileIndex == null) p._activeFileIndex = defaultFileIndex(p);
				applyActiveFile(p, p._activeFileIndex);
			}
			const genoFilename =
				p.fileName ??
				p.finalUrl ??
			...
			return `
				<tr>
					...
					<td style="max-width:200px;">
						<div class="text-truncate" style="max-width:190px;" title="${escapeHtml(filename)}">${filenameHtml}</div>
						${multiFile ? `<div class="btn-group btn-group-sm mt-1" role="group" aria-label="Switch file">
							<button type="button" class="btn btn-outline-secondary py-0 px-1 file-prev" data-id="${pid}" title="Previous file" ${p._activeFileIndex <= 0 ? 'disabled' : ''}>&lsaquo;</button>
							<span class="btn btn-outline-secondary py-0 px-1 disabled" title="File ${p._activeFileIndex + 1} of ${pFiles.length}">${p._activeFileIndex + 1}/${pFiles.length}</span>
							<button type="button" class="btn btn-outline-secondary py-0 px-1 file-next" data-id="${pid}" title="Next file" ${p._activeFileIndex >= pFiles.length - 1 ? 'disabled' : ''}>&rsaquo;</button>
						</div>` : ''}
					</td>
					<td>${published}</td>
					<td>${profileHtml}</td>
		...
		// Per-row file pager: switch which file (of multiple) is shown for a participant
		container.querySelectorAll('.file-prev, .file-next').forEach((btn) => {
			btn.addEventListener('click', () => {
				const id = btn.dataset.id;
				const p = list.find(it => String(it.id ?? it.participant_id ?? it.name) === id);
				if (!p) return;
				const files = Array.isArray(p.files) ? p.files : [];
				const cur = p._activeFileIndex ?? defaultFileIndex(p);
				const next = btn.classList.contains('file-next') ? cur + 1 : cur - 1;
				if (next < 0 || next >= files.length) return;
				applyActiveFile(p, next);
				// Keep the selection map in sync (same object reference, but re-set for clarity)
				if (selectedUsersMap.has(id)) selectedUsersMap.set(id, p);
				renderPage();
			});
		});
...
//# sourceMappingURL=displayUsers-rTrOzWZp.mjs.map
