...
function renderPgsTable(scores, targetId, title, key) {
	...
	const renderPage = () => {
		...
		scoresDiv.innerHTML = `
			...
			<div id="selectedPgsSummary_${key}" class="small text-muted mb-2">${selectedIds.size} of ${MAX_SELECTION} models selected</div>
		...
		rowCheckboxes.forEach((cb) => {
			...
			cb.addEventListener("change", () => {
				...
				if (selectedPgsSummary) {
					selectedPgsSummary.textContent = `${selectedIds.size} of ${MAX_SELECTION} models selected`;
				}
				updateGlobalSelectionCount();
				// Notify participants table of PGS selection change
				window.onPgsSelectionChange?.();
...
//# sourceMappingURL=displayScores-CvXPSduM.mjs.map
