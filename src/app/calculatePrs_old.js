...
/**
 * Get cached parsed PGS scoring file for a score
 * Uses the "PGS_Catalog:id-*" pattern so the existing Clear PGS Cache button works
 * @param {string} pgsId - PGS ID
 * @returns {Promise<Object|null>} Cached parsed score or null
 */
async function getCachedScore(pgsId) {
	try {
		const key = `PGS_Catalog:id-${pgsId}`;
		const cached = await localforage.getItem(key);
		if (cached) {
			console.log(`Using cached score for ${pgsId}`);
			return cached?.data ?? cached;
		}
		return null;
	} catch (err) {
		console.warn(`Failed to read score cache for ${pgsId}:`, err);
		return null;
	}
}

/**
 * Cache parsed PGS scoring file for a score
 * Uses the "PGS_Catalog:id-*" pattern so the existing Clear PGS Cache button works
 * @param {string} pgsId - PGS ID
 * @param {Object} parsed - Parsed PGS scoring data
 */
async function setCachedScore(pgsId, parsed) {
	try {
		const key = `PGS_Catalog:id-${pgsId}`;
		await localforage.setItem(key, parsed);
		console.log(`Cached score for ${pgsId} (${parsed?.dt?.length ?? 0} variants)`);
	} catch (err) {
		console.warn(`Failed to cache score for ${pgsId}:`, err);
	}
}
...
async function loadFallbackScores() {
	...
	// Fetch and parse each score's local file (with caching)
	let cachedCount = 0;
	const parsePromises = toLoad.map(async (score, idx) => {
		const filePath = score?.local_file;
		...
		try {
			...
			if (cached) {
				...
				return { score, parsed: cached };
			}

			// Not cached - fetch and parse
			if (statusEl) statusEl.textContent = `Fetching ${score.name || score.id}... (${idx + 1}/${toLoad.length})`;
			console.log(`Fallback score NOT CACHED, Fetching score for ${score.id} from filePath:`, filePath);
			const parsed = await loadLocalPGSFile(score.id, filePath);
			console.log(`Parsed score filePath:`, filePath);
	...
	if (resultsDiv) {
		const rows = loadedScores.map((score, idx) => {
			const id = escapeHtml(score?.id ?? "");
			const name = escapeHtml(score?.name ?? "");
			const trait = escapeHtml(score?.trait_reported ?? "");
			const variants = escapeHtml(score?.variants_number ?? "");
			const date = escapeHtml(score?.date_release ?? "");
			const loadedTxt = window.loadedPgsTxts.find(t => (t?.id ?? t?.meta?.pgs_id) === score.id);
